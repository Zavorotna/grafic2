        ...
        <table>
            ...
            <tbody>
                ...
                foreach ($_SESSION["save"] as $group => $value) {
                    ...
                    echo "<tr class='curator'>";
                    echo "<td><button>"$curator["name"]"</button></td>";
                    }
...
<script>
  let curators = document.querySelectorAll('.curator');
  for (let i = 0; i < curators.length; i++) {
    curators[i].addEventListener('click', function() {
      let students = this.parentNode.querySelectorAll('tr');
      for (let j = 0; j < students.length; j++) {
        if (students[j].style.display === 'none') {
          students[j].style.display = '';
        } else {
